...
for (col_name in colnames(alpha_diversity)) {
  tryCatch({
    ...
    col_type_status[[col_name]] <- "secced"
  }, error = function(e) {
    col_type_status[[col_name]] <- paste("failed，reason：", e$message)
  })
}
...
diversity_measures <- unique(c( 
  "Bacteria_Shannon", "Archaea_Shannon", "Fungi_Shannon"
))
...
for (factor in env_factors) {
  ...
  for (diversity in diversity_measures) {
    ...
    if (!is.numeric(alpha_diversity[[factor]]) || !is.numeric(alpha_diversity[[diversity]])) {
      warning(paste("1", factor, "or", diversity, "none"))
      next
    }
...
combined_plot <- grid.arrange(grobs = plots_list, nrow = 1, ncol = 3)

ggsave(filename = file.path(output_dir, "combined_Spearman_plots.pdf"), plot = combined_plot, width = 9.6, height = 2.4, units = "in")

ggsave(filename = file.path(output_dir, "combined_Spearman_plots.tiff"), plot = combined_plot, width = 9.6, height = 2.4, units = "in", dpi = 300)
