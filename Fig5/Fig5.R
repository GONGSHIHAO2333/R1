...
write.csv(results, file.path(output_dir, "Fig5.csv"), row.names = FALSE)

combined_plot <- grid.arrange(grobs = plots_list, nrow = 2, ncol = 3)

ggsave(filename = file.path(output_dir, "Fig5.pdf"), plot = combined_plot, width = 10, height = 5, units = "in")

ggsave(filename = file.path(output_dir, "Fig5.tiff"), plot = combined_plot, width = 10, height = 5, units = "in", dpi = 300)
